...
%>
				...
				<aside class="app-sidebar toggle-sidebar">
					<div class="app-sidebar__user pb-0">
						<div class="user-info">
							<a href="#" class="ml-2"><span class="text-dark app-sidebar__user-name font-weight-semibold"><%=userName %></span><br>
								<span class="text-muted app-sidebar__user-name text-sm"> Partner</span>
							</a>
						</div>
					</div>
											...
											<%  if(lastaction.equals(menukey)){ %>
											 <li class="slide is-expanded">
											 		<% if(JSPUtilities.getPartnersSubMenu().get(menukey)!=null){ %>
														<a class="side-menu__item active" data-toggle="slide" href="#"><i class="<%=JSPUtilities.getPartnersMenuIcons().get(i)%>"></i><span class="side-menu__label" id='idnavmenu_<%=StringUtils.replace(menuvalue, " ", "")%>'><%=menuvalue %></span><i class="angle fa fa-angle-right"></i></a>
													<% }else { %>
														<a class="side-menu__item active" href="#" onclick="javascript:fnGoToSubMenuPageJQ('<%=menuvalue%>', '<%=menukey%>');return false;"><i class="<%=JSPUtilities.getPartnersMenuIcons().get(i)%>"></i><span class="side-menu__label" id='idnavmenu_<%=StringUtils.replace(menuvalue, " ", "")%>'><%=menuvalue %></span></a>
													<% } %>
											<% }else{ %>
											<li class="slide">
													<% if(JSPUtilities.getPartnersSubMenu().get(menukey)!=null){ %>
														<a class="side-menu__item" data-toggle="slide" href="#"><i class="<%=JSPUtilities.getPartnersMenuIcons().get(i)%>"></i><span class="side-menu__label" id='idnavmenu_<%=StringUtils.replace(menuvalue, " ", "")%>'><%=menuvalue %></span><i class="angle fa fa-angle-right"></i></a>
													<% }else { %>
														<a class="side-menu__item" href="#" onclick="javascript:fnGoToSubMenuPageJQ('<%=menuvalue%>', '<%=menukey%>');return false;"><i class="<%=JSPUtilities.getPartnersMenuIcons().get(i)%>"></i><span class="side-menu__label" id='idnavmenu_<%=StringUtils.replace(menuvalue, " ", "")%>'><%=menuvalue %></span></a>
													<% } %>
											<% } %>
